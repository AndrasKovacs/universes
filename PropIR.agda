...
module IR-Univ (lvl : WO) where
  ...
  module InternalLevelTT where
    ...
    Comp t u γ = prf (unprf (u γ) ∘ unprf (t γ))

    lift : ∀ {Γ}{i j : Level Γ} → Tm Γ {j} (Lt i j) → Ty Γ i → Ty Γ j
    lift {i = i}{j} p A γ = ↑ (unprf (p γ)) (A γ)

    Cumulative : ∀ {Γ i j p A} → Tm Γ A ≡ Tm Γ (lift {Γ}{i}{j} p A)
    Cumulative {Γ}{i}{j}{p}{A} = (λ f → ∀ x → f x) & ext λ γ → El↑ (unprf (p γ)) (A γ) ⁻¹
...
-- Additional assumption: ordinal-bounded levels
...
record Ordinal (lvl : WO) : Set where
  open WO lvl
  field
    cmp    : ∀ i j → Prf (i < j) ⊎ Prf (j < i) ⊎ (i ≡ j)
    <-ext  : ∀ {i j} → (∀ k → Prf (k < i → k < j) × Prf (k < j → k < i)) → i ≡ j
  ...
  ≤-prop {p = inj₁ p}    {inj₁ q}    = refl
  ≤-prop {p = inj₁ p}    {inj₂ refl} = ⊥-elim (acyclic (unprf p))
  ≤-prop {p = inj₂ refl} {inj₁ q}    = ⊥-elim (acyclic (unprf q))
...
--   lvl = record {
--       Lvl = ℕ
--     ; _<_ = λ n m → Squash (n < m)
--     ; _∘_ = λ {(squash p) (squash q) → squash (<-trans q p)}
--     ; wf  = λ {i} → {!<-wellFounded i!}
