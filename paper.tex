...
theory with cumulative subtyping by translation to coercions in TTFL.
  ...
  %% data _≤_ (i : Lvl) (j : Lvl) : U i → U j → U (
  %% U≤ : (p : i ≤ j) → U i ≤ U j
  %% Π≤ : A ≤


%% TTFL does not directly support a
%% cumulative subtyping relation. However, it is possible to model subtyping by
%% translation to coercions in TTFL. For example:
%% \begin{itemize}
%%   \item Given $p : i < j$, we may coerce terms from type $\U\,i\,k\,q$ to $\U\,j\,k\,(p \circ q)$,
%%         using $\Lift\,p$.
%%   \item If there is a coercion from $A$ to $A'$, and
