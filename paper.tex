...
\begin{alignat*}{5}
  ...
  & \hspace{2em}\bm{\Bool} = \lambda\,\gamma.\,\Bool'
\end{alignat*}
$\bm{\Lift}_{i}^{j}p$ preserves type formers by Lemma \ref{lem:liftprop}. We
define basic term formers using metatheoretic term formers, e.g.\ $\bm{\true}_i
...
%% To verify that we have strict inclusions, we need to
%% show that $\bm{\Lift}$ and $\bm{\up}$ preserve all type and term
%% formers, but first we look at the semantics of type and term formers.

%% To verify that we have strict inclusions, it
%% remains to show that $\bm{\Lift}$ and $\bm{\up}$ preserve all type and term
%% formers.  For $\bm{\Lift}$ this follows from Lemma \ref{lem:liftprop}. For
%% $\bm{\up}$, note that semantics terms
...
%% The strict inclusions are given as follows.
