...
\end{mylemma}
\begin{proof} From the
preservation of context extension by $F$, we know that $F$ is invertible on the zero De Bruijn
index, so for $\q : \Tm\,(\Gamma \ext F\,A)\,(F (A[\p]))$, we have $F^{-1}\q : \Tm\,(\Gamma \ext
F\,A)\,(A[\p])$, such that $F (F^{-1}\,\q) = \q$. If we have any $t : \Tm\,\Gamma\,(F\,A)$, we also
have $(F^{-1}\q)[\id,\,t] : \Tm\,\Gamma\,A$, and this yields the inverse action $F^{-1}$ which acts
on all terms.
\end{proof}
