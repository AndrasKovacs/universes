...
$\U\,(\mkLvl\,i)\,?\,?$, where we have to find a level which is larger than
\emph{every} level. The solution to this issue is to simply add more levels. For
example, for polymorphism over finite levels, we may pick $\omega + \omega$ as
the first limit ordinal which can internally type finite level polymorphism;
this is what Agda 2.6.2 does.
\end{itemize}
...
Universes:
Martin-Löf (U in U), Girard, Martin-Löf (countable predicative),
CoC, Coq, Luo ECC. Universe poly in Agda (?), in Coq, cumulativity,
Sterling, Coquand normalization (?)

Transfinite: super, higher-order, Mahlo, IR, indexed IR

Harper univ checking?
