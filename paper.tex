...
\item In Section \ref{sec:ttfl} we describe type theories with internal types
  ...
  predicative universe features in Coq and Agda can be justified in our framework.
\end{enumerate}
...
\todo{ordinal indexed levels and lubs}
...
\begin{mydefinition}[IR codes]
  Assume $i : \Lvl$ and $f : (j : \Lvl) \to j < i \to \Set0$. We define
  $\uir$ and $\elir$ by simultaneous IR. For basic type formers, below we only
  include codes for function types and Booleans; other type formers are
  straightforward to add (and we do have more in our Agda formalization).
\begin{alignat*}{5}
  & \uir   &&: \Set0
...
\begin{alignat*}{3}
  ...
  & \ult\,\{j\}\,\{\acc\,f\}\,j\,p = \uir_{j\,(\ult\,\{j\}\,\{f\,j\,p\})}
\end{alignat*}
...
\begin{mylemma} Properties of $\Lift$:
\begin{enumerate}
...
\subsection{Model of TTGU}

We give a model of TTGU in this section.

\todo{disambiguate IR and TTGU type notation}

\begin{mydefinition}[Base category]
This is simply the category of sets and functions in $\Set0$, i.e.\ $\Con = \Set0$,
$\Sub\,\Gamma\,\Delta = \Gamma \to \Delta$, and the terminal object is $\top$.
\end{mydefinition}

\begin{mydefinition}[Family diagram]
We map $i : \Lvl$ to a family structure as follows.
\begin{alignat*}{5}
  &\Ty_i\,\Gamma = \Gamma \to \U\,i \hspace{2em} \Tm_i\,\Gamma\,A = (\gamma : \Gamma) \to \El\,(A\,\gamma)
\end{alignat*}
Type and term substitution are given by composition with some $\sigma : \Gamma
\to \Delta$ function. Context extension is given by $\Gamma \ext A = (\gamma :
\Gamma) \times \El\,(A\,\gamma)$. Since we define $\blank\!\ext\!\blank$ as a
$\Sigma$-type, the comprehension structure for it follows immediately, as
$\Sigma$ pairing and projections.

Mapping $i$ to a strict inclusion is defined as follows. We
\end{mydefinition}
